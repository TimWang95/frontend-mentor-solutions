# Frontend Mentor - Article preview component

This is a solution to the [Article preview component challenge on Frontend Mentor](https://www.frontendmentor.io/challenges/article-preview-component-dYBN_pYFT).

## Screenshot

![screenshot](./design/srceenshot.png)

## Links

- Solution URL: [GitHub repo](https://github.com/TimWang95/frontend-mentor-solutions/tree/main/04-article-preview/)
- Live Site URL: [solution Demo](https://timwang95.github.io/frontend-mentor-solutions/04-article-preview/)

## Built with
- ![HTML5](https://camo.githubusercontent.)
- ![CSS3](https://camo.githubusercontent.com/e6b67b27998fca3bccf4c0ee479fc8f9de09d91f389cccfbe6cb1e29c10cfbd7/68747470733a2f2f696d672e736869656c64732e696f2f62616467652f637373332d2532333135373242362e7376673f7374796c653d666f722d7468652d6261646765266c6f676f3d63737333266c6f676f436f6c6f723d7768697465)
- ![JavaScript](https://camo.githubusercontent.)

## What I Learnd

- Usage of **FLEX** lyaouts in css
- Add pseudo-class `:hover` and `:checked` to change color when you selected rating button. 
- The submit button connects to the rating button and display the popup.

## Author
- [![My GitHub](https://camo.githubusercontent.)](https://github.com/TimWang95)
- [![@TimWang95](https://camo.githubusercontent.)](https://www.frontendmentor.io/profile/TimWang95)



## Acknowledgments

- Challenge was provided by [Frontend Mentor](https://www.frontendmentor.io/home)